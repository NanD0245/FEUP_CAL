    ...
    for (int i = 0; i < vp.size(); i++) {
        for (int j = i + 1; j < vp.size(); j++) {
            if (vp[i].distance(vp[j]) < min_distance) {
                ...
                min_distance = vp[i].distance(vp[j]);
                res.p1 = vp[i];
                res.p2 = vp[j];
            }
            if (min_distance == 0)
...
Result nearestPoints_BF_SortByX(std::vector<Point> &vp) {
    ...
    for (int i = 0; i < vp.size(); i++) {
        for (int j = i + 1; j < vp.size(); j++) {
            ...
            else if (abs(vp[i].x - vp[j].x) > min_distance)
                break;
        }
    }
    return res;
...
static Result np_DC(std::vector<Point> &vp, int left, int right, int numThread) {
    if (right <= left)
        return {};
    if (right - left == 1)
        return {vp[left].distance(vp[right]),vp[left],vp[right]};

    int middle = (right + left)/2;

    Result resL = np_DC(vp,left,middle,1);
    Result resR = np_DC(vp,middle,right,1);
...
static Result np_DC_MT(std::vector<Point> &vp, int left, int right, int numThread) {
    if (right <= left)
        return {};
    if (right - left == 1)
        return {vp[left].distance(vp[right]),vp[left],vp[right]};
    ...
    if(numThreads > 1) {
        std::thread t([&]() {resL = np_DC_MT(vp, left, middle,numThread/2);});
        resR = np_DC_MT(vp, middle, right,numThread/2);
        t.join();
    } else {
        resL = np_DC_MT(vp, left, middle,numThread), resR = np_DC_MT(vp, middle, right,numThread);
    }
...
Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    sortByX(vp, 0, vp.size()-1);
    return np_DC_MT(vp,0,vp.size() - 1, numThreads);
}
